...
export function ResultsTable() {
  ...
  useEffect(() => {
    function handlePhoneAnalyzed(event) {
      setResults(event.detail)
    }

    // Add event listener
    document.addEventListener("phoneAnalyzed", handlePhoneAnalyzed)

    // Clean up
    return () => {
      document.removeEventListener("phoneAnalyzed", handlePhoneAnalyzed)
    }
  }, [])
  ...
  return (
    <div>
      ...
      <div className="flex justify-end">
        <div className="max-w-lg bg-amber-50 border-l-4 border-amber-500 p-4 rounded-lg shadow-sm text-sm text-slate-700 italic">
          ...
          <p className="text-right mt-2 font-medium text-emerald-700">- Minh Hoàng</p>
        </div>
      </div>
    </div>
  )
